...
```{r workflow, fig.cap = workflow_cap,  echo = FALSE}
...
The `compute_model` code looks in the synced folder and determines if the all the sites' data has been submitted and the updated estimate can be computed for the next iteration.  Once that is computed, the new estimates are used by the sites in `estimate_model` and the process is repeated until convergence.   A `tolerance` argument exists to control the tolerance required to define convergence, which is equal to $10^{`r log(formals(api_setup_model)[["tolerance"]], base = 10)`}$.
...
```{r docs, fig.cap = docs_cap, echo = FALSE}
...
In this example, the model converged in $`r i`$ iterations.  In practice, each site would simply run `api_estimate_model`, which wraps both the `api_submit_gradient` and `api_get_current_beta` functionality, but we separate them here as we are running multiple sites worth of data.  The output `eta` contains estimates, of the model, but we should use `api_model_converged` to extract the converged model.  Using `api_model_converged` allows us to get an object similar to the `glm` object in `R`, which the `true_model` for easier comparison.  
...
```{r tpfigcap, echo = FALSE}
traceplot_cap = paste0("Plot of coefficients over iterations. ",
                      "We present the coefficients over a number of iterations, separated by each variable.  The blue line represents the estimate of the coefficient from the full-dataset model, which we want to estimate.")
```

```{r traceplot, fig.cap = traceplot_cap, echo = FALSE}
